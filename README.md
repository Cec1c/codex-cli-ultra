...
`ccu-manager` 是 CCU 的 Ratatui 管理界面。`0.2.0-alpha.5` 的左侧导航只保留 `📦 版本与安装`、`💬 语言包`、`🎨 主题包` 和 `🔌 网络代理` 四页，不再提供首页和关于页。它覆盖版本检查、本地 fork 安装、完整 CCU 自更新、语言包/主题同步、代理和安全卸载；网络与文件任务在后台线程执行，详见 [CCUM Next TUI 迁移说明](docs/CCUM_NEXT_TUI.md)。

当新版 Codex 的自动升级提示调用 `ccu-manager upgrade quick --target <version>` 时，会直接打开跨平台快捷更新菜单，而不是强制进入全屏 Manager；稳定版仍兼容旧的 `ccu-manager --upgrade --target <version>` 调用。快捷菜单提供自动检测代理并更新（推荐）、直接更新、手动配置代理并更新、退出。自动模式按 `7890 / 10809 / 10808 / 7891 / 1080 / 2080 / 2081` 的顺序检查监听端口，并且只接受 Clash、Mihomo、v2rayN、Xray、sing-box 等常见代理进程；失败时自动回退到手动代理。下载过程使用 CCU Sky 蓝显示进度条、百分比、字节、速度和 ETA。
...
| 通道 | 当前版本示例 | 更新条件 |
| --- | --- | --- |
| CCU | `v0.2.0-alpha.5` | 安装器、管理器、内容包或文档发生变化 |
