...
- `ccu.hermes` 四段式彩色状态栏在全新安装时默认启用；前景强调色采用 Macchiato 风格的玫红、薰衣草、青绿、橙色与金色，不改终端背景或界面结构。交互安装可输入 `n` 关闭，升级和重装保留已有选择。启用后自动备份并写入 `[tui].status_line`，禁用或卸载时安全恢复，用户后续修改不会被覆盖。
