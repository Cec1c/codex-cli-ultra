...
function resolvedStableForkPackage(provider) {
...
test("an Alpha manager applies a stable package when its bundled fork is newer", async () => {
  const installRoot = await mkdtemp(join(tmpdir(), "ccu-alpha-fork-update-"));
  const report = await stageCcuUpgrade({
    installRoot,
    currentVersion: "0.2.0-alpha.4",
    currentForkVersion: "0.148.0",
    resolveCcuUpdatePackage: async () => resolvedStableForkPackage({
